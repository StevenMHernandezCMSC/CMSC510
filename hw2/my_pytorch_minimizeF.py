...
def f(w):
    ...
    # f(w)=sum(y-(x*w)
    # l = len(w)
    # out = np.array([torch.pow(torch.mul(torch.pow(x, i+1),w[i+1]),2) for i in range(l-1)])
    # return sum(out - y)

    # pred = torch.mul(x*x*x,w[3]) + torch.mul(x*x,w[2]) + torch.mul(x,w[1]) + w[0]
